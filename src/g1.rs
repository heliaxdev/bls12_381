...
impl G1Affine {
	...
	const L: [u8;16] = [
	    85, 85, 85, 85, 0, 0, 0, 0, 86, 225, 85, 85, 0, 140, 108, 57,
	    ];
        sigma = sigma.multiply_by_small(&L) - sigma2;
        sigma.is_identity()
    }
...
impl G1Projective {
    ...
    fn multiply_by_small(&self, by: &[u8; 16]) -> G1Projective {
        let mut acc = G1Projective::identity();

        // This is a simple double-and-add implementation of point
        // multiplication, moving from most significant to least
        // significant bit of the scalar.
	// The scalar is expected to be half the max size.
        //
        // We skip the leading bit because it's always unset for Fq
        // elements.
        for bit in by
